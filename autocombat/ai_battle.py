...
DELAY_PRELIM = 0                    # Seconds between turns in prelim rounds
DELAY_KNOCKOUT = 0.4               # Seconds between turns in knockout/Bo7
SHOW_ROUND_HP_KNOCKOUT = True       # Show per-round per-player HP in knockout
...
RANDOM_BONUS_2V2 = 16               # Extra random 2v2 bonus matches
...
class AIController:
    Q_TABLE = None  # Shared across all instances, loaded once

    @classmethod
    def load_q_table(cls):
        if cls.Q_TABLE is None:
            cls.Q_TABLE = load_q_table()
            if cls.Q_TABLE:
                total = sum(len(v) for v in cls.Q_TABLE.values())
                chars = list(cls.Q_TABLE.keys())
                battle_log('info', f'Q-table loaded: {total} entries for {len(chars)} chars ({", ".join(chars[:5])}...)')
                if total == 0:
                    battle_log('info', '========================================')
    ...
    def _q_decide(self, engine, user_id, ck):
        ...
        # Check if Q-table is loaded and trained for this character
        if AIController.Q_TABLE is None or ck not in AIController.Q_TABLE:
            return self._rule_decide(engine, user_id)
        qdict = AIController.Q_TABLE[ck]
...
class BattleEngine(FullBattleEngine):
    def __init__(self, fast_mode=False, delay=0.4, quiet=False, show_round_hp=False, max_rounds=None):
        super().__init__()
        ...
        self.show_round_hp = show_round_hp  # Show per-round HP in knockout
    ...
    def run_battle(self, ai_map):
        """AI-driven tournament battle loop with detailed logging."""
        if self.quiet:
            set_terminal_quiet(True)
        _bl = (lambda level, msg: None) if self.fast_mode else battle_log
        try:
...
class Tournament:
    ...
    def battle(self, team_a, team_b, map_size, phase_label='', best_of=1, fast_mode=False, delay=0.4, show_round_hp=False, max_rounds=None):
        ...
        engine = BattleEngine(fast_mode=fast_mode, delay=delay, show_round_hp=show_round_hp, max_rounds=max_rounds)
        for c in ALL_CHARACTERS:
    ...
    def _best_of_n(self, team_a, team_b, map_size, phase_label, n=7):
        ...
        for game in range(1, n + 1):
            try:
                winner, loser, rds, _ = self.battle(team_a, team_b, map_size, f'{phase_label} G{game}', best_of=n, delay=DELAY_KNOCKOUT, show_round_hp=SHOW_ROUND_HP_KNOCKOUT, max_rounds=MAX_ROUNDS_KNOCKOUT)
        ...
        for map_size in map_sizes:
            ...
            for i, j in itertools.combinations(range(len(teams)), 2):
                for rr in range(2):
                    ...
                    winner, loser, rds, _ = self.battle(ta, tb, map_size, f'{label_prefix} RR {map_size}')
    ...
    def run_phases(self):
        ...
        # Random bonus matches
        random_bonus = []
        for _ in range(RANDOM_BONUS_1V1):
            a, b = random.sample(singles, 2)
            random_bonus.append((f'1v1 Bonus', '10x10', [a], [b],
                                 singles.index(a), singles.index(b)))
        p1_pool = random_bonus + p1_pool  # Bonus first
        rrc = '单' if RR_ROUNDS_1V1==1 else ('双' if RR_ROUNDS_1V1==2 else f'{RR_ROUNDS_1V1}')
        battle_log('info', f'[1v1] {rrc}循环{len(p1_pool)-RANDOM_BONUS_1V1}场 + {RANDOM_BONUS_1V1}场随机 = {len(p1_pool)}场')

        # Phase 2: 2v2 — all C(12,2)=66 pairs, randomly shuffled
        all_pairs_2v2 = list(itertools.combinations(singles, 2))
        random.shuffle(all_pairs_2v2)
        teams_2v2 = [list(p) for p in all_pairs_2v2[:16]]  # 16 teams (random subset)
        p2_pool = self._build_rr_pool('2v2', teams_2v2, ['8x8', '10x10'], '2v2', rr_rounds=RR_ROUNDS_2V2)
        # Random bonus matches
        random_bonus_2v2 = []
        for _ in range(RANDOM_BONUS_2V2):
            a_idx, b_idx = random.sample(range(len(teams_2v2)), 2)
            random_bonus_2v2.append((f'2v2 Bonus', '10x10', teams_2v2[a_idx], teams_2v2[b_idx], a_idx, b_idx))
        p2_pool = random_bonus_2v2 + p2_pool
        rrc = '单' if RR_ROUNDS_2V2==1 else ('双' if RR_ROUNDS_2V2==2 else str(RR_ROUNDS_2V2))
        battle_log('info', f'[2v2] {len(teams_2v2)}队 {rrc}循环{len(p2_pool)-RANDOM_BONUS_2V2}场 + {RANDOM_BONUS_2V2}场随机 = {len(p2_pool)}场')

        # Phase 3: 3v3 — RR
        teams_3v3_raw = [
            ['Y1','Y10','Y11'], ['Y5','Y6','Y7','Y8'], ['Y2','Y4','Y3'],
            ['Y9','Y12'], ['Y3','Y5','Y6','Y7'], ['Y2','Y5','Y10'],
            ['Y2','Y11','Y6'], ['Y8','Y1','Y2'], ['Y3','Y4','Y10'],
            ['Y11','Y5','Y6'], ['Y7','Y8','Y1'], ['Y2','Y3','Y4'],
            ['Y9','Y3'], ['Y10','Y6','Y2']
        ]
        teams_3v3 = teams_3v3_raw
        p3_pool = self._build_rr_pool('3v3', teams_3v3, ['10x10', '20x10'], '3v3', rr_rounds=RR_ROUNDS_3V3)
        # Random bonus matches
        random_bonus_3v3 = []
        for _ in range(RANDOM_BONUS_3V3):
            a_idx, b_idx = random.sample(range(len(teams_3v3)), 2)
            random_bonus_3v3.append((f'3v3 Bonus', '10x10', teams_3v3[a_idx], teams_3v3[b_idx], a_idx, b_idx))
        p3_pool = random_bonus_3v3 + p3_pool
        rrc = '单' if RR_ROUNDS_3V3==1 else ('双' if RR_ROUNDS_3V3==2 else str(RR_ROUNDS_3V3))
        battle_log('info', f'[3v3] {len(teams_3v3)}队 {rrc}循环{len(p3_pool)-RANDOM_BONUS_3V3}场 + {RANDOM_BONUS_3V3}场随机 = {len(p3_pool)}场')
        ...
        if use_parallel:
            ...
            with ThreadPoolExecutor(max_workers=PRELIM_WORKERS) as executor:
                for args in full_pool:
                    ...
                    fut = executor.submit(self.battle, ta, tb, map_size, label, 1, True, 0)
                    future_map[fut] = args
                # Collect results
                completed = 0
            ...
            for phase_tag, label, map_size, ta, tb, i, j in full_pool:
                completed += 1
                try:
                    winner, loser, rds, result = self.battle(ta, tb, map_size, label, fast_mode=True, delay=DELAY_PRELIM)
                except Exception as e:
                    battle_log('error', f'  !! 战斗异常跳过: {label} — {e}')
                    import traceback
        ...
        def _build_top8_with_playin(ranked_list, get_team_fn, label, map_size='10x10'):
            if len(ranked_list) >= 12:
                ...
                # Weighted random pairing: 8 challengers (ranks 5-12) into 4 pairs
                challengers = [get_team_fn(ranked_list[i][0]) for i in range(4, 12)]
                # Shuffle, then pair up (weighted by rank proximity)
                random.shuffle(challengers)
                pairs = [(challengers[2*i], challengers[2*i+1]) for i in range(4)]
                battle_log('info', label + ' 5-12 play-in Bo' + str(BO_N_PLAYIN) + ':')
                for pi, (ta, tb) in enumerate(pairs):
                    cn_a = self._team_display(ta); cn_b = self._team_display(tb)
                    battle_log('info', '  Pair ' + str(pi+1) + ': ' + cn_a + ' vs ' + cn_b)
                battle_log('info', '(抽签结果展示30秒...)')
                time.sleep(30)
                new_challengers = []
                for pi, (ta, tb) in enumerate(pairs):
                    winner = self._best_of_n(ta, tb, map_size, label + ' Playin' + str(pi+1), n=BO_N_PLAYIN)
                    new_challengers.append(winner.split('+') if '+' in winner else [winner])
                return seeds + new_challengers
    ...
    def write_results(self):
        ...
        lines.append(''); lines.append('--- 战斗详情 ---')
        for a, b, winner, rounds, map_sz, phase, is_bo7 in self.results:
            bo7_tag = ' [Bo7]' if is_bo7 else ''
            lines.append(f'  [{map_sz}]{bo7_tag} {a} vs {b} → {winner} ({rounds}回合)')
        ...
        def run_custom():
            ...
            try:
                ...
                for g in range(1, num + 1):
                    engine = BattleEngine(delay=CUSTOM_BATTLE_DELAY, quiet=True)
