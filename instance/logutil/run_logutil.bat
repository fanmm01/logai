...
:: Story Painter (log coloring) upload config
set CFG_STORY_PAINTER_URL=
set CFG_STORY_PAINTER_TOKEN=
:: ============================================================
...
if !errorlevel! equ 0 (
    ...
    echo [TIP] Run manually: %PYTHON_EXE% -m pip install flask requests PyPDF2 pymupdf websockets
)
echo.
...
if not "%CFG_STORY_PAINTER_URL%"==""  set CLI_ARGS=%CLI_ARGS% --story-painter-url "%CFG_STORY_PAINTER_URL%"
if not "%CFG_STORY_PAINTER_TOKEN%"=="" set CLI_ARGS=%CLI_ARGS% --story-painter-token "%CFG_STORY_PAINTER_TOKEN%"

:: --- Launch server ---
echo [INFO] Starting LogUtil + Bridge server (Ctrl+C to stop)...
echo.
cd /d "%~dp0"
<NUL ( %PYTHON_EXE% backend.py %CLI_ARGS% %* )
