...
// @version      5.0.0
// @description  合并 Log 分析与模组分析 v5.0.0，新增 logutil 命令与 del_paren 选项，兼容 HTTP 桥接与本地群文件。
...
if (!ext) {
    ext = seal.ext.new('log-analyzer', 'Air', '5.0.0');
...
function expandShortAlias(raw) {
    ...
    // Multi-char: check if it's a fastname like TG1, AB0
    let mcm = s.match(/^([A-Za-z]{2,})(-?\d+)(?:-(\d+))?$/);
    if (mcm) {
        let pfx = mcm[1]; let n = mcm[2]; let g = mcm[3];
    ...
    // Single-char: lookup or use as category prefix
    let km = s.match(/^([A-Za-z])(-?\d+)(?:-(\d+))?$/);
    if (km) {
        let pfx = km[1].toUpperCase(); let n = km[2]; let g = km[3];
    ...
    // SealDice stripped: file-0→[file]-0
...
async function processLogTask(ctx, msg, cmdArgs, modeName, pythonMode) {
    ...
    try {
        ...
        while (maxRetries > 0) {
            ...
            if (sData.status === 'done' || sData.status === 'error') {
                // get_text 模式：文件已自动上传到群
                if (getText && sData.text_key) {
                    seal.replyToSender(ctx, msg, `✅ AI 分析完成，文本结果已发送到群（${sData.text_filename || 'ai_analysis.txt'}）`);
                    ...
                    // 兜底：文件已自动上传到群
                    seal.replyToSender(ctx, msg, `✅ AI 分析完成，文本结果已发送到群`);
                } else {
...
cmdBridge.solve = async (ctx, msg, cmdArgs) => {
    ...
    try {
        if (op === 'on') {
            ...
            let argsIdx = 2;
            while (true) {
                let arg = cmdArgs.getArgN(argsIdx);
                if (!arg) break;
                let lower = arg.toLowerCase();
                if (['permanent','audio','mod','file','link','history'].includes(lower)) {
                    saveSort = lower; argsIdx++; continue;
                }
                saveTargets.push(arg); argsIdx++;
            }
            if (saveTargets.length === 0) {
                seal.replyToSender(ctx, msg, fw(`用法: .bridge ${op} <F1/L2/H3/文件名...> [类型=P]\n将指定引用保存到目标类型。`));
                return seal.ext.newCmdExecuteResult(true);
            }
            let resp = await fetch(`${host}/api/bridge_save`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({group_id: pureId, targets: saveTargets, sort: saveSort})
            });
            let data = await resp.json();
            if (data.status === 'ok') {
                ...
                seal.replyToSender(ctx, msg, fw(`保存失败: ${JSON.stringify(data)}`));
            }
        } else if (op === 'new') {
            // v5.0.0: 新建自定义类型
            let name = cmdArgs.getArgN(2) || '';
            let fastname = cmdArgs.getArgN(3) || '';
            let persist = 'perm', scope = 'group', method = '';
            for (let i = 4; i <= 7; i++) {
                let a = (cmdArgs.getArgN(i) || '').toLowerCase();
                if (a === 'temp') persist = 'temp';
                else if (a === 'global') scope = 'global';
                else if (a === 'raw') method = 'raw';
            }
            if (!name) {
                seal.replyToSender(ctx, msg, fw('用法: .bridge new <名称> [简称] [temp/perm] [global/group] [raw]'));
                return seal.ext.newCmdExecuteResult(true);
            }
            let resp = await fetch(`${host}/api/bridge_new`, {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({group_id: pureId, name, fastname, persist, scope, method})
            });
            let data = await resp.json();
            if (data.status === 'ok') {
                seal.replyToSender(ctx, msg, fw(`已创建类型 [${data.name}]${data.fastname ? ' (简称:'+data.fastname+')' : ''} persist=${data.persist} scope=${data.scope}`));
            } else {
                seal.replyToSender(ctx, msg, fw(`创建失败: ${data.msg || JSON.stringify(data)}`));
            }
        } else if (op === 'keylist') {
            // v5.0.0: 列出所有类型
            ...
            if (data.status === 'ok') {
                ...
                for (let c of (data.categories || [])) {
                    let fn = c.fastname ? ` (${c.fastname})` : '';
                    lines.push(fw(`  [${c.name}]${fn} - ${c.count}项 | ${c.scope} | ${c.persist}`));
                }
                seal.replyToSender(ctx, msg, lines.join('\n'));
            } else {
                seal.replyToSender(ctx, msg, fw(`获取失败: ${JSON.stringify(data)}`));
...
cmdTranslate.help = '翻译桥接缓存文件。\n用法: .translate [goal-ALL] [target_lang=中文] [file1] …… [fileN]\nfile格式: [file]-N / [link]-N\n支持格式: docx, pdf, txt等。';
cmdTranslate.solve = async (ctx, msg, cmdArgs) => {
    ...
    // v4.4.1: Parse: goal-ALL first, then lang, then file/link refs
    ...
    try {
        // Get bridge file AND link list
        let listData = await safeFetchJson(`${getBackendBaseUrl()}/bridge/list?group_id=${pureGroupId}`, {}, "获取桥接文件列表");
        if (!listData || listData.status !== 'ok') {
            seal.replyToSender(ctx, msg, '❌ 当前群没有桥接缓存。请先上传文件或链接。');
            return seal.ext.newCmdExecuteResult(true);
        }
        ...
        if (resolvedFiles.length === 0) {
            seal.replyToSender(ctx, msg, '❌ 未能解析任何有效文件。请使用 .bridge list 查看可用文件/链接编号。');
            return seal.ext.newCmdExecuteResult(true);
        }
        ...
        for (let rf of resolvedFiles) {
            ...
            try {
                ...
                if (transResp.status === 'ok' && transResp.id) {
                    ...
                    transTextdbUrl = transResp.textdb_url || '';
                }
            } catch (e) {
            ...
            // v4.4.2: 翻译开始即发送确认消息
            if (isGoalAll && transTextdbUrl) {
                seal.replyToSender(ctx, msg, `${rf.name}: 开始 goal-ALL 翻译为${targetLang}\n在线查看进度: ${transTextdbUrl}`);
            } else if (!isGoalAll) {
                seal.replyToSender(ctx, msg, `${rf.name}: 开始翻译为${targetLang}，请稍候...`);
            }

            // Poll for translation result
            let translated = false;
            // v4.4.1: goal-ALL no timeout; standard keeps 60*2s=120s timeout
            let maxRetries = isGoalAll ? Number.MAX_SAFE_INTEGER : 60;
            while (maxRetries > 0) {
                await sleep(2000);
                ...
                if (sData && sData.status === 'done') {
                    ...
                    if (!isGoalAll) {
                        results.push(`${rf.name}: ✅ 翻译完成（目标语言: ${targetLang}）${textdbInfo}`);
                    } else {
                        results.push(`${rf.name}: ✅ goal-ALL 翻译完成（目标语言: ${targetLang}）${textdbInfo}`);
                    }
                    translated = true;
                    break;
                    ...
                    translated = true;
                    break;
                } else if (sData && sData.status === 'cancelled') {
                    results.push(`${rf.name}: 翻译已被停止 - ${sData.text || ''}`);
                    translated = true;
                    break;
                } else if (isGoalAll && sData && sData.textdb_url && sData.completed_chunks !== undefined) {
                    // goal-ALL progress update (logging only, don't spam the chat)
                    // Already shown initial TextDB link above
                }
                if (!isGoalAll) maxRetries--;
            }
            if (!translated) {
