...
**v4.3**
1. 文件输入扩展：`.aiutil` 及所有文件输入命令，提供文件允许为任何可展开成文本的文件（.py/.c/.js/.class 等）。
2. 文件大小上限从 50MB/40MB 统一提升至 150MB。
3. `.bridge get` 新增专用后端接口 `/api/bridge_get`，修复大文件提取失败的问题。
4. 修复非白名单扩展名文件报"不支持的文件格式"的问题，未知扩展名直接当作文本解码。
5. 修复 `FileBridgeMode=0`（WS实时推送模式）下仍启动HTTP轮询的问题，WS模式下彻底禁用HTTP轮询。
6. `.bridge get`/`.aiutil get_text`/`.translate` 改为后端自动调用 NapCat 上传文件到群，前端不再参与文件发送逻辑。
7. 新增 `raw` 修饰符（对 logutil 生效）：跳过消息头解析，直接拼接原始文本，并修复其初始失效问题。
8. 翻译结果统一以 `.txt` 扩展名发送。
9. 移除 `.ai` 作为 `.aiutil` 的别名，避免与其他插件命名冲突。
10. 修复 Windows 启动脚本编码、Python 路径嵌套、依赖安装失败等问题，确认 LLBot 兼容性。
...
2. 新增着色器链接文本自动缓存（`[link]-N`，上限30个），任何功能中使用的染色器链接自动保存为桥接缓存。
3. 新增 Web 管理界面（`/bridge/gui`）：以表格展示文件/链接/历史记录，含命令输入框，v4.4.5改为复选框多选。
4. 新增历史记录系统：文件/链接溢出项自动移入历史记录（上限50条），关闭后端时自动持久化，下次启动恢复。
5. 新增 `.bridge del` 命令：删除指定桥接项，索引自动顺延。
6. 新增短别名：`F14`/`L0`/`H23`（分别等价于 `[file]-14`/`[link]-0`/`[history]-23`），所有命令可用。
7. 新增跨群访问语法：`F0-群号`/`[file]-0-群号` 等，可在任意命令中访问其他群的桥接缓存。
8. 新增 `.halt` 命令（全员可用）：强制停止当前群内所有进行中的 AI 生成任务。
9. 新增 `goal-ALL` 翻译模式：分句逐块翻译，每 10 秒上传进度至 TextDB.online，启动时即发在线查看链接，无超时限制。
10. 新增 `.bridge master` 命令：单独查看 Web 管理界面地址（使用内网 IP 而非 127.0.0.1），彻底移除输出中的所有 127.0.0.1。
11. 新增 URL 黑名单：QQ 多媒体 CDN（`multimedia.nt.qq.com.cn`）不再自动缓存。
12. 新增 `.translate` 简写：`ALL` 等价于 `goal-ALL`。
13. `.bridge list` 改为合并聊天记录格式呈现，条目附带内容开头预览字符，v4.4.5 全部采用合并格式并新增 `all` 参数同时显示文件/链接/历史。
14. `.bridge get` 直接发送纯文本文件到群，不再附带下载链接；v4.4.2 起删除所有用户侧下载链接。
15. 消息中的染色器链接自动监听并缓存（v4.4.2）。
16. `[history]-N` 引用全面支持所有命令（`.aiutil`/`.logai`/`.translate`/`.模组分析`/`.logutil` 复合及独立发送）。
17. 修复 `bridge_link` 在复合命令中静默失效的问题（来源白名单遗漏）。
18. 修复跨群数据泄露：所有历史记录访问点增加 `group_id` 过滤隔离。
19. 修复 SealDice 去括号导致 `[link]-N`/`[history]-N` 无法识别的问题。
20. 修复 logutil 复合命令与独立发送中不识别短别名的问题：后端新增 `expand_short_alias`，前端重分词后补做别名展开。
21. 修复 `raw` 修饰符三层失效问题（DB行确保、复合op解析、WS消息处理均检查 raw 标记）。
22. 修复 `.aiutil get_text` 模式下 txt 文件可能因竞态条件而丢失的问题。
23. 修复 `.bridge get [history]-N` 与列表显示编号不一致的问题（全局→顺序计数器）。
24. 日志输出净化：NapCat 文件上传日志不再打印完整 base64 文件内容。
...
3. Web 管理界面改造：群号输入框改为复选框，自动列出全部有数据的群组，支持多选同时查看。
4. 更新版本号至 v4.4.5。