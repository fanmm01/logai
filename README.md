...
**修饰符 `get_text`**：适用于所有生成图片的指令（`.logai`、`.ai`、模组命令）。携带时，输出不再以图片方式给出，而是全部AI输出放入一个txt文件中，并发出该文件及其下载链接。
...
将桥接缓存中的文件提交给AI进行分析，提示词为输入的`prompt`。  
...
**`get_text`**：针对所有生成格式是一张图片的指令（如 `.logai`、`.ai`、模组命令）。在此修饰被带有时，输出不再以图片方式给出，而是把全部AI输出放在一个txt文件里，并发出该文件及其下载链接。
...
**v4.2.0**  
1. 新增 `.ai` 命令：快速AI分析，仅支持 `[file]-N` 格式文件，不保存配置。无文件时仅将提示词交给AI。
