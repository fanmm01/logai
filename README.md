...
`.bridge list [file|link|history]`  返回桥接缓存列表（v4.4.0: 支持分类筛选，默认同时显示文件和链接）。  
`.bridge get [file]-N/[link]-N/[history]-N`  返回编号文件转纯文本后的txt下载链接（v4.4.0更新格式）。  
`.bridge del [file]-N [link]-N ...`  (v4.4.0新增) 删除指定编号的桥接项，索引自动顺延。
...
### 四、其他
`.搜模组 <关键词>`  百度网盘搜索模组  
`.translate [target_lang=中文] [file1] …… [fileN] [goal-ALL]`  调用翻译接口，对docx/pdf/txt等格式文件进行翻译，返回翻译后的txt文件。file1-fileN 为 `[file]-N` 或 `[link]-N` 格式（v4.4.0新增）。默认目标语言为中文。翻译结果同时上传到群并保留下载链接。  
v4.4.0新增 `goal-ALL` 参数：分句翻译，每10秒将进度上传至 TextDB.online 云数据库，可在线查看。支持 `.halt` 暂停。

`.halt` (v4.4.0新增)  强制停止当前群内所有进行中的AI生成任务。仅骰主可用。
...
### 八、Web管理界面 (v4.4.0新增)
`.bridge list` 时会显示网页版管理地址（`/bridge/gui/<群号>`），可在浏览器中查看所有文件、链接、历史记录的表格，并支持输入命令。
